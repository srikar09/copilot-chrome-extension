...
function Chat({ baseContext, sampleQuestions, secondaryContext }: ChatProps) {
  ...
  const [currentContext, setCurrentContext] = useState<ContextTypes>(baseContext);
  const [toast, setToast] = useState<React.ReactElement | null>();
  const sendMessage = async (
    message: string,
  ) => {
    setLoading(true);
    const promptGenerator = new PromptContext(currentContext.context, userAccount);
              ...
              {initialAnalyticMessage.length < 2 && (
                <>
                  <span className="mx-auto flex flex-grow text-gray-800 dark:text-gray-800 clear-both">
                    Pick an option below to get started ! If you dont pick anything you can still ask questions against your general finances ! Please pick only one option at a time
                  </span>
              ...
              {secondaryContext?.map((secondaryContext) => (
                <div key={secondaryContext.contextName} className="flex items-center space-x-2 py-3">
                  <Switch
                    ...
                    onClick={() => {
                      if (currentContext.contextName !== baseContext.contextName || (currentContext.contextName !== secondaryContext.contextName &&  currentContext.contextName !== baseContext.contextName) ) {
                        setCurrentContext(baseContext)
                      }
                      else {
                        if(Object.keys(secondaryContext.context).length === 0){ 
                          setToast(<Toast 
                            show= {true} 
